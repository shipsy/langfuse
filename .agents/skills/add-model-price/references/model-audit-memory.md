...
| Anthropic | claude-opus-4-5-20251101 | Same as Opus 5 | Yes | Flat 1M context | Yes | None | https://platform.claude.com/docs/en/about-claude/pricing | Re-confirmed unchanged (page row: "Claude Opus 4.5"). |
...
| Anthropic | claude-opus-4-20250514 | Input $15/MTok, Output $75/MTok, 5m $18.75/MTok, 1h $30/MTok, read $1.50/MTok | Yes | Retired — no tiering | Not applicable | None | https://platform.claude.com/docs/en/about-claude/pricing | Re-confirmed present on current page's main table, unchanged. Page now reads "retired, except on Google Cloud" (dropped the Bedrock exception wording seen in prior audits) — deprecation-availability text only, not a pricing dimension Langfuse represents. |
| Anthropic | claude-sonnet-5 | Input $2/MTok, Output $10/MTok; 5m $2.50/MTok, 1h $4/MTok, read $0.20/MTok | Yes | Flat 1M context | Yes | None | https://platform.claude.com/docs/en/about-claude/pricing | **Resolved:** the official page now states the $2/$10 pricing "is now the standard price" and the previously scheduled Sept 1 2026 increase to $3/$15 "will not occur". This closes the long-standing unresolved finding from prior audits — do not re-flag a pending price increase for this model. |
...
| AWS Bedrock | claude-3-5-sonnet-20240620 / claude-3.5-sonnet-20241022 (Public Extended Access SKU) | $6.00/MTok input, $30.00/MTok output, $7.50/MTok cache write, $0.60/MTok cache read | Yes (confirmed real Aug 4 2026) | Distinct dated SKU, not a context-length tier | Not applicable | Unresolved | https://aws.amazon.com/bedrock/pricing/ | Not re-verified this run; permanent documented limitation (model-ID string match cannot distinguish billing SKU) — see provider-sources-and-price-keys.md. |
...
| OpenAI | gpt-5-chat-latest | Input $1.25/MTok, Cached $0.125/MTok, Output $10/MTok | Yes | No provider tiering | Not applicable | None | https://developers.openai.com/api/docs/models/gpt-5-chat-latest | Freshly re-verified this run via dedicated model page (previously flagged "not re-verified" across several prior audits). Matches existing file value exactly. |
...
| OpenAI | davinci-002 | Input $2.00/MTok, Output $2.00/MTok (base/non-fine-tuned inference); no cache | Yes | No large-context tier | Yes | None | https://developers.openai.com/api/docs/pricing | Re-confirmed via full table dump; matches the Standard/base-model table rate fixed in the Aug 7 2026 audit. See provider-sources-and-price-keys.md for the base-vs-fine-tuning-legacy pricing lesson. |
| OpenAI | babbage-002 | Input $0.40/MTok, Output $0.40/MTok (base/non-fine-tuned inference); no cache | Yes | No large-context tier | Yes | None | https://developers.openai.com/api/docs/pricing | Re-confirmed via full table dump; matches the Standard/base-model table rate fixed in the Aug 7 2026 audit. |
...
| Google | gemini-3.5-flash-lite | Input $0.30/MTok, Output $2.50/MTok, Cache read $0.03/MTok | Yes | No large-context tier | Yes | None | https://ai.google.dev/pricing | Re-confirmed via a targeted verbatim fetch that explicitly separated Free-tier ("Not available") from Paid-tier ("$0.03") for the context-caching cell — matches the file's existing $0.03/MTok cache-read price. A same-run broad (non-targeted) fetch again showed "Not available" for this cell, repeating the exact free/paid column-collapse artifact documented since July 2026; the targeted verbatim fetch remains the reliable check for this specific cell. |
...
No newly released flagship text/chat/reasoning model was found on any of the three
providers' official pricing pages this run (each fetch was explicitly asked to flag any
model name not already tracked; none were flagged). No pricing-file or `types.ts` change
was made in the August 12 2026 audit.

## Unresolved findings (updated 2026-08-12)
...
6. **Base vs. fine-tuning legacy pricing confusion is a real historical bug class** — The
   davinci-002/babbage-002 fix from the Aug 7 2026 audit revealed that OpenAI's pricing
...
7. **Legacy/embedding/base-completion catalog tail not covered this run** — Entries such as
